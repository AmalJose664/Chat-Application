...
function ListUsers() {
	...
	return (
		<div className="home-users-container">
			<div
				className="home-refresh users"
				onClick={() => {
					setConversationsStore([], 0)
					getConversations()
				}}
			>
				<Refresh color="white" />
			...
			<div className="home-users">
				{convLoader && (
					<motion.div className="list-friends-loader" initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.2 }}>
						<l-tail-chase size="28" speed="1.8" color="white"></l-tail-chase>
					</motion.div>
				)}
				{conversations.length != 0
					? conversations.map((value, i) => {
							if (value.other_user) {
								return (
									<div
										key={i}
										className={value.other_user.sqlite_id == selectedUserId ? "home-list-users-each selected" : "home-list-users-each"}
									>
										<EachConversations
											value={value}
											i={i}
											key={i}
											type={!!convListTyping[value.other_user.sqlite_id]}
					...
					: !convLoader && (
							<motion.p initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.2 }} className="">
								Start a new Chat
							</motion.p>
					  )}
			</div>
...
const EachConversations = React.memo(({ value, i: index, type, isOnline }) => {
	...
	return (
		<motion.div
			key={index}
			className={value.unread_count != 0 ? "home-user unread" : "home-user"}
			initial={{ opacity: 0, x: -60 }}
			animate={{ opacity: 1, x: 0 }}
			transition={{ duration: 0.5, delay: index / 10, ease: "easeInOut" }}
		>
			<div className="home-user-cont">
				<div className={isOnline ? "home-user-pic is-online" : "home-user-pic"} onClick={() => showMyPic(value.other_user)}>
					<img src={value.other_user.profile_picture} alt="" />
				</div>
				<div className="home-user-detail" onClick={() => changeUser(value, index, value.unread_count)}>
					<div className="user-top">
						<div className="user-title">{value.other_user.user_name}</div>
						{value.unread_count != 0 && <div className="user-unread">{value.unread_count}</div>}
					</div>
					<div className="user-bottom">
						<div className="user-last-m">
							...
							{type ? (
								<motion.div
									className="list-covns-user-istyping"
									initial={{ opacity: 0, y: 20 }}
									animate={{ opacity: 1, y: 0 }}
									transition={{ duration: 0.2 }}
								>
									<l-leapfrog size="20" speed="2" color="#78c5f4"></l-leapfrog>
								</motion.div>
							) : (
								<motion.span initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.2 }}>
									{lastMessage(value.lst_m)}
								</motion.span>
							)}
						</div>
						<div className="user-last-status">
							<div className="show-time-user-list-conv-list">{getTime(value.t, false)}</div>
							{filterLastMessage(value.lst_m, authUser.db_user._id) ? (
								<>
									<ReadReceipts status={value.l_s} />
									{/* <p>{value.l_s}</p> */}
								</>
							) : (
								""
